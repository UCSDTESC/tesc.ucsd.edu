...
function Event(props) {
    ...
    return (
        <div className="col-md-3 align-self-center d-flex flex-column p-3 events__card">
            ...
            {props["Event Name"]}
            {withWrapper(<img className="align-self-center justify-self-center" alt={props["Event Name"]} 
                    src={props["Image"] !== undefined && props["Image"][0].url} style={{maxHeight: "8rem", width: "auto", maxWidth: "100%"}}/>)}
...
function Events() {
    ...
    useEffect(() => {
        base.current('Council Org Events')
            ...
            .then(records => records.sort((a, b) => {return (new Date(a["Start Date"])- new Date(b["End Date"]))}))
    ...
    function renderCurrEvents() {
        ...
        currEvents = currEvents.filter((event) => {
            ...
            return ((new Date(event["Start Date"]) - new Date(event2["Start Date"])));
        });
